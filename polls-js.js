...
function poll_process() {
	if(pollsL10n.show_fading) {
		jQuery('#polls-' + poll_id).fadeOut('def', function () {
...
function poll_process_success(data) {
	...
	if(pollsL10n.show_fading) {
		jQuery('#polls-' + poll_id).fadeIn('def', function () {		
			set_is_being_voted(false);	
		});
	} else {
		set_is_being_voted(false);	
...
// Disable Poll's Voting Booth
function poll_disable_voting(current_poll_id){
	jQuery(document).ready(function () {
		jQuery('#polls_form_1 :radio, #polls_form_1 :checkbox, #polls_form_1 :button').each(function(i){
